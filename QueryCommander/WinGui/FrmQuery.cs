...
namespace QueryCommander
{
	...
	public class FrmQuery : FrmBaseContent
	{
		...
		private QueryCommander.Config.Settings _settings;
		...
		public FrmQuery(Form mdiParentForm)
		{
			...
			_settings = QueryCommander.Config.Settings.Load();
		...
		protected override void Dispose( bool disposing )
		{
			...
			_settings.ShowEOLMarkers=qcTextEditor.ShowEOLMarkers;
			_settings.ShowSpaces=qcTextEditor.ShowSpaces;
			_settings.ShowTabs=qcTextEditor.ShowTabs;
			_settings.ShowLineNumbers=qcTextEditor.ShowLineNumbers;
			_settings.ShowMatchingBracket=qcTextEditor.ShowMatchingBracket;

			_settings.fontFamily = qcTextEditor.Font.FontFamily.Name;
			_settings.fontGraphicsUnit = qcTextEditor.Font.Unit;
			_settings.fontSize = qcTextEditor.Font.Size;
			_settings.fontStyle = qcTextEditor.Font.Style;
		...
		private void HandleExecutionResult(DataSet ds, TimeSpan executionTime, IDatabaseManager db)
		{
			...
			try
			{
				...
				if(ds != null)
				{
					...
					if(ds.Tables.Count==0)
					{
						AddToRecentObjects(GetObjectName());
		...
		public void SetDatabaseConnection(string dbName, IDbConnection conn)
		{
			...
			else if(conn is Sybase.Data.AseClient.AseConnection)
			{
				try{conn.ChangeDatabase(dbName);}
				catch
				{
					conn = new Sybase.Data.AseClient.AseConnection(conn.ConnectionString);
					conn.Open();
					conn.ChangeDatabase(dbName);
				}
				DatabaseName = dbName;
		...
		public void RunQuery()
		{
			...
			try
			{
				...
				_currentDataSet = db.ExecuteCommand_DataSet (SQLstring, dbConnection,DatabaseName);

				executionTime = DateTime.Now.Subtract(dt);
				HandleExecutionResult(_currentDataSet,executionTime,db);
				return;
